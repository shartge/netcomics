...
B<netcomics> [B<-aAbBDGhiIlLoPsuvv>] [B<-[c|C]> I<"comic ids">] [B<-p> I<url>]
  [B<-S,-T,-E> I<date>] [B<-R> retries] [B<-n,-N> I<days>] [B<-d,-m,-t> I<dir>]
  [B<-g> [I<program>]] [B<-nD>] [B<-r> I<rc_file>] [B<-P>] [B<-W,-w>[=I<n>]] [B<-nw>]
  [B<-[n]wa>] [B<-wt> I<"webpage title">] [B<-f,-wf> I<"date fmt">]
...
all of the websites updated.  Use B<-N> if you want to run the
...
show_comics(1) comicpage(1) strftime(3)
